...
class ResNet_Block(e2nn.EquivariantModule):
    def __init__(self, in_channels, out_channels,):
        super().__init__()

        self.relu = e2nn.ReLU(out_channels)
        self.double_conv = e2nn.R2Conv(in_channels, out_channels, kernel_size=5, padding=2)
        # 这里使用e2nn.R2Conv替代nn.Conv2d
        self.shortcut = e2nn.R2Conv(in_channels, out_channels, kernel_size=1)

    def forward(self, inputs):
        x1 = self.double_conv(inputs)
...
class ResUNet_eq(nn.Module):
    def __init__(self, n_channels, n_classes, bilinear=True):
        super(ResUNet_eq, self).__init__()
        ...
        self.down1 = ResNet_Block(feat_type_down1, feat_type_down2)
        self.down2 = ResNet_Block(feat_type_down2, feat_type_down3)
        self.down3 = ResNet_Block(feat_type_down3, feat_type_down4)
        self.down4 = ResNet_Block(feat_type_down4, feat_type_down5)
        ...
        self.lin1 = nn.Linear(2004480, 512)
